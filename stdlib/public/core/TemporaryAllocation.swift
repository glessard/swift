...
@_alwaysEmitIntoClient @_transparent
internal func _fallBackToHeapAllocation<
  E: Error, R: ~Copyable
>(
  byteCount: Int,
  alignment: Int,
  ...
  do throws(E) {
    return .success(try body(buffer._rawValue))
  } catch {
    return .failure(error)
  }
}
...
>(
  ...
  let result = _withUnsafeTemporaryAllocation(
    of: Int8.self,
    capacity: byteCount,
    alignment: alignment
  ) {
    pointer throws(E) -> R in
...
>(
  ...
  let result = _withUnprotectedUnsafeTemporaryAllocation(
    of: Int8.self,
    capacity: byteCount,
    alignment: alignment
  ) {
